...
from .serializers import (ProblemCreateSerializer, ProblemDetailSerializer,
                          ProblemListSerializer, ProblemTagSerializer)
...
class ProblemListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_authenticated:
            ...
            if solved == "true":
                solved_problem_ids = (
                    self.request.user.submission_set.filter(status="accepted")
...
class UserProblemStatsView(APIView):
    ...
    def get(self, request):
        ...
        solved_problem_ids = solved_submissions.values_list(
            "problem_id", flat=True
        ).distinct()

        solved_easy = Problem.objects.filter(
            id__in=solved_problem_ids, difficulty="easy"
        ).count()

        solved_medium = Problem.objects.filter(
            id__in=solved_problem_ids, difficulty="medium"
        ).count()

        solved_hard = Problem.objects.filter(
            id__in=solved_problem_ids, difficulty="hard"
        ).count()
...
class RandomProblemView(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            {"error": "No problems found"}, status=status.HTTP_404_NOT_FOUND
        )

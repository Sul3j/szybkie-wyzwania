...
accepted_submissions = Submission.objects.filter(
    status='accepted',
    points_awarded=0
).select_related('user', 'problem').order_by('submitted_at')

print(f"Znaleziono {accepted_submissions.count()} zaakceptowanych submissions bez punktów\n")
...
for submission in accepted_submissions:
    ...
    if key not in users_awarded:
        ...
        print(f"✓ Przyznano {submission.points_awarded} punktów użytkownikowi {submission.user.username} za zadanie '{submission.problem.title}'")
...
for user in User.objects.all():
    ...
    print(f"{user.username}: {profile.experience_points} punktów, poziom {profile.level}, ranga {profile.rank}")

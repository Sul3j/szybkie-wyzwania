...
class UserProfile(models.Model):
    ...
    def _update_level(self):
        for level, threshold in sorted(settings.LEVEL_THRESHOLDS.items(), reverse=True):
            if self.experience_points >= threshold:
                self.level = level
                break

    def _update_rank(self):
        for threshold, rank in sorted(settings.RANKS.items(), reverse=True):
            if self.experience_points >= threshold:
                self.rank = rank
                break
    ...
    def solved_count(self):
        return (
            self.user.submission_set.filter(status="accepted")
            .values("problem")
            .distinct()
            .count()
        )
    ...
    def acceptance_rate(self):
        if self.total_submissions_count == 0:
            return 0.0
        return round(
            (self.accepted_submissions_count / self.total_submissions_count) * 100, 2
        )

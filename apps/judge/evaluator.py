    ...
    def evaluate(self):
        ...
        try:
            ...
            return {
                ...
                "error_message": (
                    self._get_error_message(test_results) if not all_passed else ""
                ),
            }
...
""",
            ...
            compilation_overhead = (
                10 if self.language in ["csharp", "cpp"] else 0
            )  # Extra 10s for compiled languages
            timeout = (
                base_timeout + compilation_overhead + 2
            )  # Base + overhead + buffer
            try:
                ...
                if result["StatusCode"] != 0:
                    ...
                    is_compilation_error = (
                        result["StatusCode"] == 2 or "COMPILATION_FAILED" in output
                    )
                    ...
                    return {
                        ...
                        "error": (
                            "Compilation failed"
                            if is_compilation_error
                            else "Program exited with non-zero status"
                        ),
                        "is_hidden": test_case.is_hidden,
                    }
                ...
                return {
                    ...
                    "expected": (
                        expected_output if not test_case.is_hidden else "Hidden"
                    ),
                    "actual": (
                        actual_output[:1000] if not test_case.is_hidden else "Hidden"
                    ),
                    "is_hidden": test_case.is_hidden,
                }
                ...
                error_str = str(e)
                is_compilation_error = (
                    "COMPILATION_FAILED" in error_str or e.exit_status == 2
                )

                return {
                    "passed": False,
                    "status": (
                        "compilation_error" if is_compilation_error else "runtime_error"
                    ),
                    "input": test_input if not test_case.is_hidden else "Hidden",
                    "error": str(e)[:500],
                    "is_hidden": test_case.is_hidden,
...
"""
        wrapper = (
            wrapper_template.replace("{user_code}", self.code)
    ...
    def _prepare_csharp_code(self, test_input):
        ...
        func_match = re.search(
            r"public\s+static\s+([\w\[\]<>,\s]+?)\s+(\w+)\s*\(([^)]*)\)", self.code
        )
        if not func_match:
        ...
        if has_class_definition:
            ...
            indented_code = "\n".join(
                "    " + line if line.strip() else line for line in code_lines
            )
            wrapper += "// User solution class\n"
        ...
        if param_count == 1:
            wrapper += (
                "            // Single argument (could be array, int, or string)\n"
            )
            wrapper += "            dynamic arg;\n"
            ...
            wrapper += (
                "            // Two arguments - smart split that respects brackets\n"
            )
            ...
            wrapper += (
                "            } else if (int.TryParse(parts[1], out int intVal2)) {\n"
            )
            ...
            wrapper += (
                "            // Three arguments - smart split that respects brackets\n"
            )
            wrapper += "            var parts = new List<string>();\n"
            wrapper += "            int depth = 0, start = 0;\n"
            wrapper += "            for (int i = 0; i < testInput.Length; i++) {\n"
            ...
            wrapper += "            }\n\n"
            wrapper += (
                "            // Parse arguments (could be array, int, or string)\n"
            )
            wrapper += "            dynamic arg1;\n"
            wrapper += '            if (parts[0].StartsWith("[")) {\n'
            wrapper += "                arg1 = System.Text.Json.JsonSerializer.Deserialize<int[]>(parts[0]);\n"
            ...
            wrapper += (
                "            } else if (int.TryParse(parts[2], out int intVal3)) {\n"
            )
            ...
            wrapper += "            }\n\n"
            wrapper += (
                f"            result = Solution.{func_name}(arg1, arg2, arg3);\n\n"
            )
        else:
            wrapper += f'            throw new Exception("Unsupported parameter count: {param_count}");\n\n'
        wrapper += (
            "            // Print result in correct format (JSON for arrays/objects)\n"
        )
    ...
    def _prepare_cpp_code(self, test_input):
        ...
        for i, line in enumerate(lines):
            ...
            if (
                ...
                potential_return = " ".join(
                    parts[:-1]
                )  # Everything else is return type

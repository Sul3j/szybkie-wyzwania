...
class AllSubmissionsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        queryset = Submission.objects.filter(status='accepted').select_related('problem', 'user')
...
class UserSubmissionStatsView(APIView):
    ...
    def get(self, request):
        ...
        status_distribution = dict(
            submissions.values('status').annotate(count=Count('status')).values_list('status', 'count')
        )

        language_distribution = dict(
            submissions.values('language').annotate(count=Count('language')).values_list('language', 'count')
        )
...
class ProblemSubmissionsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Submission.objects.filter(
            ...
            status='accepted'
        ).select_related('problem', 'user').order_by('execution_time')
